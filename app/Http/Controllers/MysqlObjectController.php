...
class MysqlObjectController extends Controller
{
    public function view_report(Request $request)
    {
        Session::put('lap_start_date', null);
        Session::put('lap_end_date', null);
        $table = Mysql_object::where('object_type', 'table')->orderBy('object_size', 'desc')->take(10)->with('database.application');
        $index = Mysql_object::where('object_type', 'index')->orderBy('object_size', 'desc')->take(10)->with('database.application');
        $tablespace = Mysql_object::where('object_type', 'tablespace')->orderBy('object_size', 'desc')->take(10)->with('database.application');
    ...
    public function export_pdf()
    {
        ...
        $table = Mysql_object::where('object_type', 'table')->whereBetween('created_at', [$start, $end])->orderBy('object_size', 'desc')->take(10)->with('database.application')->get();
        $index = Mysql_object::where('object_type', 'index')->whereBetween('created_at', [$start, $end])->orderBy('object_size', 'desc')->take(10)->with('database.application')->get();
        $tablespace = Mysql_object::where('object_type', 'tablespace')->whereBetween('created_at', [$start, $end])->orderBy('object_size', 'desc')->take(10)->with('database.application')->get();
        ...
        $pdf = PDF::setOptions(['dpi' => 150, 'defaultFont' => 'sans-serif', 'isRemoteEnabled' => true])
            ->loadView('export.pdf_mysql_object', compact('table', 'index', 'tablespace', 'url', 'today'));
        return $pdf->stream();
    }
}
